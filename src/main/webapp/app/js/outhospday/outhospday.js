...
layui.config({
    ...
    var $ = layui.$
        ...
        ,table = layui.table;
    table.render({
        elem: '#outhospdayTable'
        ,url: $WEB_ROOT_PATH+'/dhccApi/outhospday/outhospday/list'
        ,cellMinWidth: 80
        ,height: tableHeight
        ,cols: [[
    ...
    form.on('submit(LAY-user-front-search)', function(data){
        ...
        //执行重载
        layui.table.reload('outhospdayTable', {
            where: field
        });
    });
    ...
    var active = {
        add: function(){
            ...
            layer.open({
                type: 2
                ,title: '添加超住院天数收费'
                ,content: $WEB_ROOT_PATH+'/outhospday/outhospdayAdd'
                ,maxmin: true
                ,area: ['750px', '450px']
                ,btn: ['确定', '取消']
                ,yes: function(index, layero){
                    ...
                    iframeWindow.layui.form.on('submit('+ submitID +')', function(data){
                        ...
                        //提交 Ajax后台
                        var url=$WEB_ROOT_PATH+"/dhccApi/outhospday/outhospday/save";
                        $.post(url,field,function(result){
                            ...
                            //后台成功后，静态更新表格中的数据
                            table.reload('outhospdayTable'); //数据刷新
    ...
    //监听行点击
    table.on('tool(outhospdayTable)', function(obj){
        var data = obj.data;
        console.log(data)
        if(obj.event === 'del'){
            //删除
            layer.confirm('确定删除该超限定疗程？', function(index){
                //执行 Ajax 后重载
                var url=$WEB_ROOT_PATH+"/dhccApi/outhospday/outhospday/delete";
                $.post(url,{'outhospday.id':data.id},function(result){
                    table.reload('outhospdayTable');
                    layer.msg('已删除');
                });
            });
