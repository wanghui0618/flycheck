...
layui.config({
	  ...
	  //日期范围
	    ...
	    table.render({
	    	elem: '#yestodayMzTable'
	            ,url: $WEB_ROOT_PATH+'/dhccApi/medicalCount/medicalCount/yestodayHospitalData?medicalVo.auditMethods=2'
	            ,cellMinWidth: 100
		...
		form.on('submit(LAY-org-front-search)', function(data){
	        ...
	        //执行重载
	        layui.table.reload('yestodayMzTable', {
	            where: field
	        });
	    });
