...
layui.config({
	  ...
	  //日期范围
	    ...
	    table.render({
	    	elem: '#yestodayUpdateTable'
	            ,url: $WEB_ROOT_PATH+'/dhccApi/medicalCount/medicalCount/yestodayUpdateData?medicalVo.auditMethods=1'
	            ,cellMinWidth: 100
		...
		form.on('submit(LAY-org-front-search)', function(data){
	        ...
	        //执行重载
	        layui.table.reload('yestodayUpdateTable', {
	            where: field
	        });
	    });
