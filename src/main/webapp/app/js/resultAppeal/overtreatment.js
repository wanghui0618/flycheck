...
layui.config({
	    ...
	    table.render({
	    	elem: '#userTable'
	            ,url: '/piccbid/app/js/resultAppeal/overtreatment.json'
	            ,cellMinWidth: 80
	           // ,where: {  }
	            ,height : tableHeight
	            ,cols: [[
	            	   {type: 'numbers',width:40, title: '序号',fixed: 'left' }
	            	  ,{field:'left', title:'操作', toolbar: '#table-useradmin-webuser'}
		              ...
		              ,{field:'hospitalCode', title: '医院编号',align:'center'}
		              ,{field:'hospitalName', title: '医院名称',align:'center'}		             
		              ,{field:'diagnose', title: '姓名',align:'center'}          		             
...
*/
		 table.on('row(userTable)', function(obj){
		      ...
		      layer.open({
		          type: 2
		          ,title: '过度医疗'
		          ,content: $WEB_ROOT_PATH+'/bigDataAntiFraud/overtreatmentInfo'
		          ,maxmin: true
		          ,area: ['850px', '420px']
		      });
		 });
