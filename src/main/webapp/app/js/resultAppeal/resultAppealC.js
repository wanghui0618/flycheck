			  ...
			  elem: '#resultAppeal'
				 ...
				 // ,height: 226
				  ...
				  ,cols: [[
					  ...
					  ,{field:'unitType',title: '操作',fixed: 'left' , width:276,align:'center',templet: function(d){
						  ...
						  if(toolbarType==1 && ((type==6||type==2)&&auditMethods==0)){
							  {{if (!existsButton('resultAppealC-agree-reject')) { }}		          
							  ...
							  {{if (!existsButton('resultAppealC-agree-reject')) { }}	
							  return '<a class="layui-btn layui-btn-xs" lay-event="agree"><i class="layui-icon layui-icon-ok"></i>接受</a><a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="reject"><i class="layui-icon layui-icon-no">&#x1006;</i>不接受</a><a  class="layui-btn layui-btn-normal layui-btn-xs" onclick="'+clickID+'"style="width:110px;background-color:#FF9933;" ><i class="layui-icon layui-icon-form"></i>材料（未审核）</a>';	
							  { }}
						  }else if(toolbarType==1 && (type==2 && auditMethods==1)){	
							  {{if (!existsButton('resultAppealC-agree-reject')) { }}	
							  return '<a class="layui-btn layui-btn-xs" lay-event="agree"><i class="layui-icon layui-icon-ok"></i>接受</a><a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="reject"><i class="layui-icon layui-icon-no">&#x1006;</i>不接受</a><a  class="layui-btn layui-btn-normal layui-btn-xs" onclick="'+clickID+'"style="width:80px"><i class="layui-icon layui-icon-form"></i>申诉材料</a>';
							  { }}
					  ...
					  ,{field:'status',title: '处理进度', width:207,fixed:'left',align:'center',templet: function(d){
						  var codex =d.status;
						  var examineComments=d.examineComments;
						  var auditMethods=d.auditMethods;
				 ...
				 if(audit_methods==0){
					 getAllViolationCost(medical_id);
					  $('#dg').datagrid('reload',{'medicalDetail.medicalId': medical_id,'medicalDetail.isIlegal':is_ilegal,'historyNumber':historyNumber}); 
					   if(appeal_status=='2'||appeal_status=='6'){
						   $('#resultAppealC-examineAll').css({ "display": "block" });
					   }else{
						   $('#resultAppealC-examineAll').css({ "display": "none" });
					   }
				 }else{
			...
			columns:[[	
				...
				,{field:'operate',title:'申诉材料',align:'center',fixed: 'left',width:100,formatter:function(value,row,index){
						   ...
						   if(audit_methods==1){						
								...
								var str ='<a style="color:#1BC445;font-weight:600;">医院接受违规</a>';
								return str;						
								...
								var str = '<a  class="layui-btn layui-btn-normal layui-btn-xs" onclick="'+clickID+'"style="width:80px;background-color:#FF9933;"><i class="layui-icon layui-icon-form"></i></i>查看材料</a>';
								return str; 
								{{ } }}	
					...
					onLoad:function(){
						...
						$.post(url,{"medicalDetail.id":row.id,"medicalDetail.medicalId":medical_id},function(result){
							...
							//修正后待验证
						     ...
						     if(appeal_status=="2" && audit_methods==0){		
...
function addInfo(itemId,status,medicalId,countTime){
	...
	if(countTime!=''&& countTime!=null){
		...
		layer.open({
			type: 2
			,title: '申诉材料(历史记录)'
				,content: $WEB_ROOT_PATH+'/resultAppeal/resultAppealInfoC'
				,maxmin: true
				,area: ['760px', '600px']
		,btn: ['确定', '取消']
...
function addInfoMain(appealId,money,reason){	
	layer.open({
		type: 2
		,title: '申诉材料及审核'
			,content: $WEB_ROOT_PATH+'/resultAppeal/resultAppealInfoMainC'
			,maxmin: true
			,area: ['760px', '600px']
	,btn: ['确定', '取消']
	,success: function(layero, index){
...
function saveItem(){
	...
	var field={"medicalDetail.id":itemId,"medicalDetail.medicalId":medicalId,"medicalDetail.deductions":deductions,"medicalDetail.withholdingAmount":withholdingAmount,"medicalDetail.withholdingQuantity":withholdingQuantity};
