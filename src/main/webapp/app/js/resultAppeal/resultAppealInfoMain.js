...
$(function () {
    ...
    uploader.on('fileQueued', function (file) {
    	if(file.type=="image/jpeg"||file.type=="image/png"||file.type=="image/gif"||file.type=="image/jpg"||file.type=="image/bmp"){
    		//图片
    		var $li = $(
    				'<div id="' + file.id + '" class="hover_img file-item thumbnail" style="width: 155px;">' +
    				'<div class="delImg" id="delImg" ></div>' +
                    '<img mysrc="" class="prev" id="' + 45678 + '">' +
                    '<div style="width: 155px;height:35px;line-height: 18px;font-size: 15px;white-space:normal;word-break:break-all;word-wrap:break-word; display: inline-block;height: 16px;line-height: 16px">' + file.name + '</div>' +
                '</div>'
    	            ),
    	            $img = $li.find('img');
    		$listImg.append( $li );
    		...
    		$list.append('<div class="webuploader-item" >' +
    	            '<div  id="' + file.id + '"  class="hover_file thumbnail" style="overflow: auto;line-height: 25px;height: 30px;">' +
    	            '<h4 style="font-size: 15px;display: inline-block;height: 24px;line-height: 16px"  onclick="checkB('+ "'"+file.name+"'"+')">' + file.name + '</h4>' +
    	            '<div class="delFile" id="delFile"></div>' +
    	            '</div>' +
    	            '</div>');
    	}
    ...
    //图片执行删除方法
    $listImg.on("click", ".delImg", function () {
        var Id = $(this).parent().attr("id");
        uploader.removeFile(uploader.getFile(Id, true));
        $(this).parent().remove();
    });
...
function saveDb(){
	var url = $WEB_ROOT_PATH + "/dhccApi/resultAppeal/resultAppeal/saveInfoMain";
	dataAll.field["resultAppeal.fileUrl"]=fileUrl;
	dataAll.field["resultAppeal.originalFilename"]=originalFilename;
	dataAll.field["resultAppeal.id"]=$("#id").val();
	dataAll.field["resultAppeal.medicalId"]=$("#medicalId").val();
	dataAll.field["resultAppeal.orgName"]=$("#orgName").val();
	dataAll.field["resultAppeal.orgCode"]=$("#orgCode").val();
	dataAll.field["resultAppeal.appealDate"]=$("#appealDate").val();
	dataAll.field["resultAppeal.appealPerson"]=$("#appealPerson").val();
	dataAll.field["resultAppeal.appealReason"]=$("#appealReason").val();
	dataAll.field["resultAppeal.examineComments"]=$("#examineComments").val();
	dataAll.field["resultAppeal.examinePerson"]=$("#examinePerson").val();
	dataAll.field["totalAmount"]=$("#totalAmount").val();
	dataAll.field["userVerifyRemarks"]=$("#userVerifyRemarks").val();
    $.post(url, dataAll.field, function (result) {
    	 var inFlag= result.operateSuccess; 
   	    window.parent.reloadLayer(inFlag);
    });
}
