			  ...
			  elem: '#resultAppeal'
				 ...
				 // ,height: 226
				  ...
				  ,cols: [[
					  ...
					  ,{field:'unitType',title: '操作',fixed: 'left' , width:180,align:'center',templet: function(d){
					...
					/*	  if(toolbarType==1 && ((type==6||type==2)&&auditMethods==0)){
						  ...
						  if(type==2 && auditMethods==0){
							  return '<span style="color:black "; >'+ "等待对方处理 "+'</span>';	
						  }else if(type==2 && (auditMethods==1 && (examineComments==null||examineComments==''))){							
							  return '<a  class="layui-btn layui-btn-normal layui-btn-xs" onclick="'+clickID+'"style="width:110px;" ><i class="layui-icon layui-icon-form"></i>材料（未审核）</a>';	
						  }else if(type==2 && auditMethods==1){
							  return '<a  class="layui-btn layui-btn-normal layui-btn-xs" onclick="'+clickID+'"style="width:110px;background-color:#D2B025E8;"><i class="layui-icon layui-icon-form"></i>材料（已审核）</a>';	
						  }else if(type==4||type==0){
							  {{if (!existsButton("resultAppealH-yes-no")) { }}
							  ...
							  {{} }}
						  }else if((type==5||type==2)&&auditMethods==0){
							  {{if (!existsButton("resultAppealH-send")) { }}
							  return '<a class="layui-btn layui-btn-normal layui-btn-xs" lay-event="send" style="background-color: #FF9933;"><i class="layui-icon layui-icon-release"></i>发送</a>'	
							  {{} }}			               
						  }else if( (type==5 && auditMethods==1) && (appealReason==null||appealReason=="")){		      
							  {{if (!existsButton("resultAppealH-send")) { }}
							  return '<a  class="layui-btn layui-btn-normal layui-btn-xs" onclick="'+clickID+'"style="width:80px ;background-color:#FF9933;"><i class="layui-icon layui-icon-form">提交材料</i></a>'	
							  {{} }}		               
						  }else if(type==5 && auditMethods==1){		      
							  {{if (!existsButton("resultAppealH-send")) { }}
							  return '<a class="layui-btn layui-btn-normal layui-btn-xs" lay-event="send" style="background-color: #FF9933;"><i class="layui-icon layui-icon-release" ></i>发送</a><a  class="layui-btn layui-btn-normal layui-btn-xs" onclick="'+clickID+'"style="width:80px ;background-color: #23D256E8;"><i class="layui-icon layui-icon-form">申诉材料</i></a>'	
							  {{} }}		               
					  ...
					  ,{field:'status',title: '处理进度', width:182,fixed:'left',align:'center',templet: function(d){
						  console.log(d);
						  var codex =d.status;
						  var examineComments=d.examineComments;
						  var auditMethods=d.auditMethods;
						  ...
						  if(codex==0){
							  ...
							  if((appealReason=="null"||appealReason==null)||(fileUrl=="null"||fileUrl==null)){
								  codex="拒绝违规，申诉材料（×）"; 
							  }else{
							  codex="拒绝违规，申诉材料（√）"; 
							  }
					  ...
					  ,{field:'sysStatus', width:85,align:'center',fixed:'left',title:'机审结果',templet: function(d){
						  var a =d.sysStatus;
						  if(a=="1"){
							  a="疑似违规";
						  }else if(a=="0"){
					  ...
					  ,{field:'userStatus', width:85,align:'center',title:'初审结果',fixed:'left',templet: function(d){   
						  var a =d.userStatus;
						  if(a=="1"){
							  a="疑似违规";
						  }else if(a=="0"){
				 ...
				 if(audit_methods==0){
					 getAllViolationCost(medical_id);
					  $('#dg').datagrid('reload',{'medicalDetail.medicalId': medical_id,'medicalDetail.isIlegal':is_ilegal,'historyNumber':historyNumber}); 
					   if( appeal_status=='5'){
						   $('#resultAppealH-examineAll').css({ "display": "block" });
					   }else{
						   $('#resultAppealH-examineAll').css({ "display": "none" });
					   }
				 }else{
					...
					var text="<span style='font-size:14px; color:red;font-weight:600;'>主病例违规，违规金额："+totalAmount+"元。</span>";
	...
	formSelects.data('select1', 'local', {
		...
		$('#dg').datagrid({		
			...
			columns:[[	
				...
				,{field:'operate',title:'申诉材料',align:'center',fixed: 'left' ,width:100,formatter:function(value,row,index){
						   ...
						   if(audit_methods==1){						
								...
								var str = '<a  class="layui-btn layui-btn-normal layui-btn-xs" onclick="'+clickID+'"style="width:80px"><i class="layui-icon layui-icon-edit"></i>历史查看</a>';
								return str; 
								{{} }}		
								...
								var str ='<a style="color:#1BC445;font-weight:600;">接受违规</a>';
								return str;						
							}else if(toolbarType==1 && (itemIdisExist==""||itemIdisExist==null)){						
								var str ='<a style="color:#F68E00;font-weight:600;">未提交</a>';
								return str;						
								...
								var str = '<a  class="layui-btn layui-btn-normal layui-btn-xs" onclick="'+clickID+'"style="width:80px"><i class="layui-icon layui-icon-form"></i></i>查看材料</a>';
								return str; 
								{{ } }}
							}else if(toolbarType==2 && ((itemIdisExist!=""&&itemIdisExist!=null)&&(status=='2'||status=='8'))){
								var	clickID="addInfo('"+row.itemId+"','"+status+"')";
								{{if (!existsButton("resultAppeal-detailsReadOnly")) { }}
								var str = '<a  class="layui-btn layui-btn-normal layui-btn-xs" onclick="'+clickID+'"style="width:80px"><i class="layui-icon layui-icon-form"></i>查看材料</a>';
								return str;
								{{ } }}
							}else if(toolbarType==2 && ((itemIdisExist!="" && itemIdisExist!=null) && status=='5')){
								var	clickID="addInfo('"+row.itemId+"','"+status+"','"+medicalId+"')";
								{{if (!existsButton("resultAppeal-detailsSubmitDone")) { }}
								var str = '<a  class="layui-btn layui-btn-normal layui-btn-xs" onclick="'+clickID+'"style="width:80px ;background-color:#23D256E8 ;"><i class="layui-icon layui-icon-form"></i>申诉材料</a>';
								return str; 
								{{} }}
							}else if(toolbarType==2 && ((itemIdisExist==""||itemIdisExist==null)&& status=='5')){
								var	clickID="addInfo('"+row.itemId+"','"+status+"','"+medicalId+"')";
								{{if (!existsButton("resultAppeal-detailsSubmit")) { }}
								var str = '<a  class="layui-btn layui-btn-normal layui-btn-xs" onclick="'+clickID+'"style="width:80px ;background-color:#FF9933 ;"><i class="layui-icon layui-icon-form"></i>提交材料</a>';
								return str; 
								{{} }}
							}else{
								var str ='<a>未提交</a>';
				...
				,{field:'appealResult',title:'公示结果',width:80,align:'center',formatter:function(value,row){
					var a=row.appealResult;
		            	if(a=="2"){
		            		a="<span style='color:#1BC445;font-weight:600;'>正常</sapn>";
		            	}else if(a=="0"||a=="3"){
		            		a="<span style='color:#ea5f5f;font-weight:600;'>违规</sapn>";
		            	}else if(a==null || a=="null"){
		            		a="未审核";
		            	}
		                return '<span >'+ a +'</span>'				
	            ...
	            ,{field:'applyPayLevel',align:'center', title: '报销级别'}
			...
			onExpandRow: function(index,row){
				...
				ddv.panel({
					...
					onLoad:function(){
						...
						console.log(row);
						$.post(url,{"medicalDetail.id":row.id,"medicalDetail.medicalId":medical_id},function(result){
			...
			/*	$('#dg').datagrid('fixDetailRowHeight',index);//固定详情页面高度
				var innerH=$('.datagrid-body-inner').height();*/
			}
		});	
	   $("#dg").datagrid('resize'); 
...
function addInfo(itemId,status,medicalId,countTime){
	...
	if(countTime!=''&& countTime!=null){
		...
		layer.open({
			type: 2
			,title: '申诉材料(历史记录)'
				,content: $WEB_ROOT_PATH+'/resultAppeal/resultAppealInfoH'
				,maxmin: true
				,area: ['760px', '500px']
		,btn: ['关闭']
...
function addInfoMain(appealId,money,reason){	
	layer.open({
		type: 2
		,title: '申诉材料及审核'
			,content: $WEB_ROOT_PATH+'/resultAppeal/resultAppealInfoMainH'
			,maxmin: true
			,area: ['760px', '600px']
	,btn: ['确定', '取消']
	,success: function(layero, index){
...
function cellClick(rowIndex, field, value){
	if(field=="operate"||field=="ck"){
		tableRowClick();
	}
}
//easyui行点击
...
function getAllViolationCost(medicalId) {
	$.getJSON($WEB_ROOT_PATH+'/dhccApi/resultAppeal/resultAppeal/getAllViolationCost?medicalDetail.medicalId='+medicalId,function(cost){				
		...
		var text="<span style='font-size:14px; color:red;font-weight:600;'>接受违规明细:"+accepetItemAmount+"条，合计:"+accepetCost+"。未确认明细:"+violationItemAmount+"条，合计:"+violationItemCost+"。</span>";
