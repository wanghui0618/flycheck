...
layui.config({
    ...
    table.render({
        elem: '#detailTable'
        ,url:$WEB_ROOT_PATH + '/dhccApi/childrensDrugs/childrensDrugs/detail'
        ,height: tableHeight
        ,method:'POST'
        ,where:{"ChildrensDrugsVo.hisid":hisid,"ChildrensDrugsVo.itemId":itemId,"ChildrensDrugsVo.itemName":itemName}
        , cols: [
            [
    ...
    form.on('submit(LAY-detailedInformation-front-search)', function (data) {
        ...
        if(field['illegal']==0){
            field['ChildrensDrugsVo.itemId']=itemId;
            field['ChildrensDrugsVo.itemName']=itemName;
        }
        //执行重载
