...
layui.config({
	    ...
	    ,table = layui.table;
	    table.render({
	    	elem: '#resolveFeeTable'
	        ,url: $WEB_ROOT_PATH+'/dhccApi/resolveFee/resolveFee/listVo'
	        ,cellMinWidth: 80
	        ,height: tableHeight
	        ,cols: [[
			 {type: 'numbers', width:40, title: '编号'}
   			,{title:'操作', width: 160, align:'center', toolbar: '#table-orgadmin-webuser',hide:rowOperate(['resolveFee-update','resolveFee-delete'])}        
			...
			,{field:'includeItemCode', width:250,align:'center',title: '关联项目编码' }
			,{field:'includeItemName', width:250,align:'center',title: '关联项目名称' }
		...
		form.on('submit(LAY-org-front-search)', function(data){
	        ...
	        //执行重载
	        layui.table.reload('resolveFeeTable', {
	            where: field
	        });
	    });
	    ...
	    var active = {
	      add: function(){
	        ...
	        layer.open({
	          type: 2
	          ,title: '添加超限额收费信息'
	          ,content: $WEB_ROOT_PATH+'/resolveFee/resolveFeeAdd'
	          ,maxmin: true
	          ,area: ['880px', '500px']
	          ,btn: ['确定', '取消']
	          ,yes: function(index, layero){
	            ...
	            iframeWindow.layui.form.on('submit('+ submitID +')', function(data){
	              ...
	              //提交 Ajax后台 
	              console.log(field);
	              var url=$WEB_ROOT_PATH+"/dhccApi/resolveFee/resolveFee/save";
	              $.post(url,field,function(result){
	            		  layer.msg('添加成功!');
	            		//后台成功后，静态更新表格中的数据
		                  table.reload('resolveFeeTable'); //数据刷新
	    ...
	    //监听行点击
	    table.on('tool(resolveFeeTable)', function(obj){
	      var data = obj.data;
	      if(obj.event === 'del'){
	    	//删除
		  layer.confirm('确定删除该信息吗？', function(index){
		      //执行 Ajax 后重载
		        var url=$WEB_ROOT_PATH+"/dhccApi/resolveFee/resolveFee/delete";
		        $.post(url,{'resolveFee.id':data.id},function(result){
		        	table.reload('resolveFeeTable');
		            layer.msg('删除成功');
		    	    });
		        });
