...
layui.config({
    ...
    //监听行单击事件（单击事件为：rowDouble）
	    table.on('rowDouble(icdTable)', function(obj){   	
	   ...
	   table.render({
		            ...
		            ,cols: [[
		            	...
		            	,{type:'status',title: '状态',width:150,align:'center',templet: function(d){
	                	    var codex =d.status;
		                	if(codex=="已关联"){
		                		 return '<span style="color:#00FF00"; >'+ codex +'</span>'	 
		                	}else if(codex=="未关联-名称相似"){
		                		return '<span style="color:#FF3300 "; >'+ codex +'</span>'	 
		                	}
	                  }}
			            ,{field:'code',width:150,align:'center', title: 'ICD编码'}
	...
	/*	//监听radio
				...
				allrelation: function(){//一键关联
					...
					if(typeName ==""  || typeName2 ==null){
						...
						return;
					      ...
					      layer.confirm('将一键关联<span style="color: red;">'+typeName+"版'</span>'与<span style='color: red;'>"+typeName2+"版'</span>'中同名ICD数据", function(index){ 	
	    ...
	    //监听搜索
		form.on('submit(icdRelation-search-left)', function(data){
		  ...
		  //监听搜索
		form.on('submit(icdRelation-search-right)', function(data){
	    	var field = data.field;    	
	    	 console.info(field);
	        //执行重载
	        layui.table.reload('icdTableTo', {
