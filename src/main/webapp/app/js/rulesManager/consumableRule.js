...
layui.config({
	    ...
	    table.render({
	    	elem: '#drugruleTable'
	            ,url: $WEB_ROOT_PATH+'/dhccApi/consumablerule/consumableRule/list'
	            /*,cellMinWidth: 80*/
	           // ,where: {  cityCode: 'DHCC'  }
	            ,height: tableHeight
	            ,cols: [[
		              ...
		              ,{field:'itemCode',width:130,align:'center', title: '耗材编码',fixed: 'left'}
		              ,{field:'itemName',width:367,align:'center', title: '耗材名称',fixed: 'left'}
		              ,{field:'consumableType', width:90,align:'center',title: '耗材分类',templet: function(d){
	                	    var codex =d.consumableType;
		                	if(codex==null||codex==""){
		                		codex="-";
		                	}
		                	if(codex=="0"){
		              ...
		              ,{field:'itemStandard',width:180,align:'center', title: '耗材规格'}
		              ,{field:'itemUnit',width:60,align:'center', title: '单位'}
		              ,{field:'consumableLevel',width:90,align:'center', title: '耗材类别'
		            	  ,templet: function(d){
		                	    var codex =d.consumableLevel;
			                	if(codex==null||codex==""){
			                		codex="-";
			                	}
			                	if(codex=="1"){
	      ...
	      add: function(){
	    	  //耗材信息新增
	    	    layer.open({
			          type: 2
			          ,title: '新增'
				          ,content: $WEB_ROOT_PATH+'/consumablerule/consumableInfo'
				          ,maxmin: true
				          ,area: ['650px', '505px']
				          ,btn: ['确定', '取消']
	                  ...
	                  ,yes: function(index, layero){
                        ...
                        //提交 Ajax后台 
			              var url=$WEB_ROOT_PATH+"/dhccApi/consumablerule/consumableRule/saveInfo";
	      ...
	      if(obj.event === 'xiugai'){
	    	  //修改
	    	    layer.open({
			          type: 2
			          ,title: '修改'
			          ,content: $WEB_ROOT_PATH+'/consumablerule/consumableRuleInfo'
			          ,maxmin: true
			          ,area: ['650px', '475px']
			          ,btn: ['确定', '取消']
	    	          ,success: function(layero, index){
			          ...
			          ,yes: function(index, layero){
			            ...
			            iframeWindow.layui.form.on('submit('+ submitID +')', function(data){
			              ...
			              //提交 Ajax后台 
			              var url=$WEB_ROOT_PATH+"/dhccApi/consumablerule/consumableRule/save";
			              $.post(url,field,function(result){	
			            	 // var inFlag= result.inFlag; 
			            	  //if(inFlag==0){
			            		  layer.msg('编辑成功!');
	    	...
	    	//删除
	    	  var itemCode=data.itemCode;
		        layer.confirm('是否确定删除', function(index){
		        	//执行 Ajax 后重载
		            var url=$WEB_ROOT_PATH+"/dhccApi/consumablerule/consumableRule/delete";
		            $.post(url,{"consumableRule.itemCode":itemCode},function(result){
	    	...
	    	//耗材信息删除
	    	  ...
	    	  var cityCode =data.cityCode;
		        layer.confirm('是否确定删除', function(index){
		        	//执行 Ajax 后重载
		            var url=$WEB_ROOT_PATH+"/dhccApi/consumablerule/consumableRule/deleteInfo";
		            $.post(url,{"consumableInfo.id":id,
		            	        "consumableRule.itemCode":itemCode,
		            	        "consumableInfo.itemCode":itemCode,
		            	        "consumableInfo.cityCode":cityCode,
		            }
		                  ,function(result){	
