...
layui.config({
	    ...
	    table.render({
	    	elem: '#userTable'
	            ,url: $WEB_ROOT_PATH+'/dhccApi/rescue/rescue/listinfo'
	            /*,cellMinWidth: 80*/
	            ,height: 415
	            ,where: {  billingNo: billing_no ,admissionNo: admission_no  }
	            ,cols: [[
	              { type: 'numbers',width:40, title: '序号'  }
	              //,{title:'操作', toolbar: '#table-useradmin-webuser', width:210}
	              ,{field:'ilegalRescue', title: '违规?', width: 80, sort: true
	                  ,templet: function(d){
	  ...
	  /*  table.on('tool(userTable)', function(obj){
	      ...
	      if(obj.event === 'changeToWeiweigui'){
		        ...
		        layer.confirm(data.itemName+'标记为未违规', function(index){
		            ...
		            $.post(url,{'rescue.billingNo':data.itemCode},function(result){
