    ...
    $('#pCategory').combogrid({
       ...
       textField: 'name',
        ...
        columns: [[
            ...
            { field: 'name', title: '费用类别', width: 80 }
        ...
        onSelect: function (d,row) {
            $('#pCategory').combogrid("setValue", row.name);
            $('#diagName').val(row.name);
        } 
    });
    
     $('#pCategory').combogrid('textbox').unbind()//先解绑所有事件，要不输入的内容找不到匹配项，回车时输入框内容会被清空
    .keydown(function (e) {
        if (e.keyCode == 13) {
            var keyValue = $('#pCategory').combogrid('textbox').val();
            var queryParams = $('#pCategory').combogrid("grid").datagrid('options').queryParams;
            queryParams.keyword = keyValue;//keyword;
            //重新加载
            $('#pCategory').combogrid("grid").datagrid("reload");

            $('#pCategory').combogrid("setValue", keyValue);
        }
    });
     $('#pCategory').combogrid('textbox').keyup(function(e){
    	 var keyValue = $('#pCategory').combogrid('textbox').val();
    	 $('#pCategory').combogrid("setValue", keyValue);
     });
     
     $(function(){
         $('#pCategory').combobox('textbox').bind('focus',function(){
            $('#pCategory').combobox('showPanel');
            $('#pCategory').combogrid("setValue", '');
            $('#diagName').val('');
        }); 
    });

    //取得分页组件对象
    var pager = $('#pCategory').combogrid('grid').datagrid('getPager');
    if (pager) {
        $(pager).pagination({
             ...
             onSelectPage: function (pageNumber, pageSize) { //按分页的设置取数据
            	var keyword =$('#pCategory').combogrid('textbox').val();
                ...
                $('#pCategory').combogrid("grid").datagrid('options').pageSize = pageSize;
            },
    ...
    var getData = function (page, pagesize,url) { 
        $.ajax({
            ...
            success: function (data) {
               $('#pCategory').combogrid("grid").datagrid("loadData", data);
            }
        }); 
