...
layui.config({
	    ...
	    table.render({
	    	elem: '#userTable'
	            ,url: $WEB_ROOT_PATH+'/dhccApi/admin/admin/AverageNumber'
	            ,cellMinWidth: 80
	             ,height: 485
	            ,cols: [[
		...
		form.on('submit(LAY-org-front-search)', function(data){
    	 ...
    	 //重新加载table的图像
 	    $.getJSON($WEB_ROOT_PATH+'/dhccApi/admin/admin/AverageNumber?inFlag='+pname+'&number='+number,
				function(data){
 	    	var name;
 	    	var pnumber;
  	    ...
  	    name =data.data[0].pname;
   	    pnumber =data.data[0].pnumber;}
			...
			$.ajax({
				url: $WEB_ROOT_PATH+'/dhccApi/admin/admin/OutDate?inFlag='+name,
				type:"get",
		...
		//默认初始值
	    $.getJSON($WEB_ROOT_PATH+'/dhccApi/admin/admin/AverageNumber',
				function(data){
