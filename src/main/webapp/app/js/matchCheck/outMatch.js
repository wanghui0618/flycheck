...
layui.config({
	    ...
	    table.render({
	    	elem: '#drugruleTable'
	            ,url: $WEB_ROOT_PATH+'/dhccApi/outMatch/outMatch/list'
	            /*,cellMinWidth: 80*/
	           //,where: {  cityCode: 'DHCC'  }
	            ,height: tableHeight
	            ,cols: [[
		              ...
		              ,{field:'itemCode',width:100,align:'center', title: '项目编码',fixed: 'left'}
	      ...
	      add: function(){
	    	  //诊疗信息修改
	    	    layer.open({
			          type: 2
			          ,title: '新增'
			          ,content: $WEB_ROOT_PATH+'/matchCheck/outMatchInfo'
			          ,maxmin: true
			          ,area: ['1000px', '415px']
			          ,btn: ['确定', '取消']
	                  ...
	                  ,yes: function(index, layero){
                        ...
                        //提交 Ajax后台 
			              var url=$WEB_ROOT_PATH+"/dhccApi/outMatch/outMatch/saveInfo";
			              //console.info(field);
	    	    ...
	    	    layer.open({
			          ...
			          ,yes: function(index, layero){
			            ...
			            iframeWindow.layui.form.on('submit('+ submitID +')', function(data){
			              var field = data.field; //获取提交的字段
			              //提交 Ajax后台 
			              var url=$WEB_ROOT_PATH+"/dhccApi/outMatch/outMatch/save";
			              $.post(url,field,function(result){	
			            		  layer.msg('编辑成功!');
				            	  //后台成功后，静态更新表格中的数据
				                  table.reload('drugruleTable'); //数据刷新
	    	...
	    	//规则删除
	    	  var itemCode=data.itemCode;
		        layer.confirm('是否确定删除', function(index){
		        	//执行 Ajax 后重载
		            var url=$WEB_ROOT_PATH+"/dhccApi/outMatch/outMatch/delete";
		            $.post(url,{"outMatch.itemCode":itemCode},function(result){
