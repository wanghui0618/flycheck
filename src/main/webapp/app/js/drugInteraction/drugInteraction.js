...
layui.config({
	...
	,table = layui.table;
	 ...
	 ,cols: [[
		 ...
		 ,{title:'操作', align:'center',toolbar: '#table-useradmin-webuser1', width:330,hide:rowOperate(['drugInteraction-drug-show','drugInteraction-interaction-add','drugInteraction-drug-update','drugInteraction-drug-delete'])}
		...
		if(obj.event === 'delete'){
			...
			,yes: function(index, layero){
				...
				iframeWindow.layui.form.on('submit('+ submitID +')', function(data){
					var field = data.field; //获取提交的字段
					/*console.log(field);
					console.log(field['drugInteraction.drugName']);*/
					//提交 Ajax后台 
					var url=$WEB_ROOT_PATH+"/dhccApi/druginteraction/drugInteraction/updateDrugName";
					$.post(url,field,function(result){
						var inFlag= result.inFlag; 
						if(inFlag==0){
