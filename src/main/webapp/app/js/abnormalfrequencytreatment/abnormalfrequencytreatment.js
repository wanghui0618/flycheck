...
layui.config({
...
}).use(['index', 'table','element','laydate','form', 'upload', 'layer'], function(){
    ...
    $.ajax({
   	 url:$WEB_ROOT_PATH+'/dhccApi/abnormalfrequencytreatmentrest/getBenefitTypemz',
		 dataType: 'json',
		 type: 'get',
		 success: function (data) {
			 $('#route').empty();
				...
				for(var i=0;i<data.length;i++){
					str += "<option value='"+data[i].BENEFIT_TYPE+"'>"+data[i].BENEFIT_TYPE+"</option>";
					/*str += "<option>"+data[i].BENEFIT_TYPE+"</option>";*/
				}
				$('#route').append(str);
				form.render('select');		 
		 }
...
function search(){
    ...
    var  formDate={route:route,insurance:insurance,hospitalCode:hospitalCode,hospitalName:hospitalName,
                    settlementTime:settlementTime,departmentName:departmentName,day:day,count:count};
...
function totle(formDate){
    $.ajax({
        url:$WEB_ROOT_PATH+'/dhccApi/abnormalfrequencytreatmentrest/getTotalNumberOfCasesAndTotalAmount',
        type: "POST",
        data:formDate,
        dataType: "json",
        async: true,
...
function Detaildg(formDate){
    ...
    table.render({
        elem: '#dg'
        ,loading:true
        ,url: $WEB_ROOT_PATH+'/dhccApi/abnormalfrequencytreatmentrest/getFrequencyTreatmentDetails'
        ,cellMinWidth: 80
        ...
        ,cols: [[
            ...
            ,{field: 'PATIENT_ID', title: '患者ID',align:'center',width:165}  
            ,{field: 'ADMISSION_DEPT_NAME', title: '科室名称',align:'center',width:165}  
            ,{field: 'CLAIM_TYPE', title: '门诊类型',align:'center',width:165}                     
            ,{field: 'ADMISSION_DISEASE_ID', title: '诊断编码',align:'left',width:200}
            ,{field: 'ADMISSION_DISEASE_NAME', title: '诊断名称',align:'left',width:200}
    ...
    table.on('tool(dg)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
        ...
        if(layEvent === 'detail'){ //查看
            ...
            layer.open({
                ...
                area: ['1100px', '550px'],
                title:'病例详情',
                content:$WEB_ROOT_PATH + '/abnormalfrequencytreatment/abnormalfrequencytreatmentDetails',//这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['http://sentsin.com', 'no']
...
function InsuredDig(formDate){
    ...
    table.render({
        elem: '#dig'
        ,loading:true
        ,url: $WEB_ROOT_PATH+'/dhccApi/abnormalfrequencytreatmentrest/getInsuredDataForm'
        ,cellMinWidth: 50
        ,loading: true  //翻页加loading
        ,height: tableHeight-55
        ,cols: [[
            //文本靠左，数字靠右，等长居中
             {type: 'numbers', fixed: 'left',title:"序号" ,fixed: 'left',align:'right'}       	       	
        	,{field: 'BENEFIT_TYPE', title: '险种类型' ,width:'15%',align:'left',width:110}
            ,{field: 'PATIENT_NAME', title: '患者姓名' ,width:'15%',align:'left',width:110}
            ,{field: 'PATIENT_ID', title: '患者ID',align:'center',width:150}
            ,{field: 'SOCIAL_CARD_ID', title: '患者社保卡号',align:'right',width:135}
            ,{field: 'COUNT', title: '就诊次数',align:'right',width:120,sort:true}
...
function downLoadXsl(fileName){
    ...
    form.attr('action',$WEB_ROOT_PATH+'/dhccApi/abnormalfrequencytreatmentrest/downLoadFile');
...
function exportD(sid){
    ...
    $.ajax({
        url:$WEB_ROOT_PATH+"/dhccApi/abnormalfrequencytreatmentrest/expDate",
        type: "POST",
        data: formDate,
        dataType: "json",
        async: true,
...
$("#export").click(function(){
        ...
        if(sid != null && sid != ""){
            ...
            formDate={route:route,insurance:insurance,hospitalCode:hospitalCode,hospitalName:hospitalName,
                settlementTime:settlementTime,departmentName:departmentName,day:day,count:count};
        }
        layer.load(2,{shade: [0.4, '#000']});
        $.ajax({
