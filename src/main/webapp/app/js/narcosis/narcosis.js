...
layui.config({
	    ...
	    table.render({
	    	elem: '#narcosisTable'
	        ,url: $WEB_ROOT_PATH+'/dhccApi/narcosis/narcosis/listVo'
	        ,cellMinWidth: 80
	        ,height: tableHeight
	        ,cols: [[
		...
		form.on('submit(LAY-org-front-search)', function(data){
	        ...
	        //执行重载
	        layui.table.reload('narcosisTable', {
	            where: field
	        });
	    });
	    ...
	    var active = {
	      add: function(){
	        ...
	        layer.open({
	          type: 2
	          ,title: '添加超限额收费信息'
	          ,content: $WEB_ROOT_PATH+'/narcosis/narcosisAdd'
	          ,maxmin: true
	          ,area: ['880px', '500px']
	          ,btn: ['确定', '取消']
	          ,yes: function(index, layero){
	            ...
	            iframeWindow.layui.form.on('submit('+ submitID +')', function(data){
	              ...
	              //提交 Ajax后台 
	              console.log(field);
	              var url=$WEB_ROOT_PATH+"/dhccApi/narcosis/narcosis/save";
	              $.post(url,field,function(result){
	            		  layer.msg('添加成功!');
	            		//后台成功后，静态更新表格中的数据
		                  table.reload('narcosisTable'); //数据刷新
	    ...
	    //监听行点击
	    table.on('tool(narcosisTable)', function(obj){
	      var data = obj.data;
	      if(obj.event === 'del'){
	    	//删除
		  layer.confirm('确定删除该信息吗？', function(index){
		      //执行 Ajax 后重载
		        var url=$WEB_ROOT_PATH+"/dhccApi/narcosis/narcosis/delete";
		        $.post(url,{'narcosis.id':data.id},function(result){
		        	table.reload('narcosisTable');
		            layer.msg('删除成功');
		    	    });
		        });
