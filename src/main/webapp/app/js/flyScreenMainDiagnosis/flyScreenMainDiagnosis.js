		...
		table.render(
			{
				...
				,page: true
				,height: tableHeight
				,url: $WEB_ROOT_PATH+'/dhccApi/flyScreenMainDiagnosis/flyScreenMainDiagnosis/list'
				,cols: [[
			...
			table.render(
				{
					...
					,url: $WEB_ROOT_PATH + '/dhccApi/flymedical/flymedical/relateDiagnosis'
					,cellMinWidth : 80
					,height : tableHeight
					,page : true
					,cols : [[
						{field:'id', title: 'ID', sort: true, hide:true}
						,{field: 'icd10NameBasy', title: '主要诊断', width:'25%'}
						,{field: 'patientName', title: '患者姓名', width:'25%'}
						,{field: 'hospitalName', title: '医疗机构', width:'25%'}
						,{field: 'totalAmountBasy', title: '医疗费用', width:'25%'}
					]]
					,where : {
			...
			function(obj){
				...
				if(obj.event == 'edit'){
					layer.open(
						{
							type : 2,
							title : '修改信息',
							content : $WEB_ROOT_PATH + '/flyScreenMainDiagnosis/add',
							maxmin : true,
							area : ['600px', '450px'],
							btn : ['确定', '取消'],
							...
							yes : function(index, layero){
								...
								iframeWindow.layui.form.on(
									...
									function(data){
										// 获取提交的字段
										var field = data.field;
										var url = $WEB_ROOT_PATH + '/dhccApi/flyScreenMainDiagnosis/flyScreenMainDiagnosis/save';
										$.post(
											url,
											field,
											function(result){
					...
					layer.confirm(
						'确定删除该条记录吗？',
						function(index){
							var url = $WEB_ROOT_PATH + '/dhccApi/flyScreenMainDiagnosis/flyScreenMainDiagnosis/delete';
							$.post(
								url,
								{
