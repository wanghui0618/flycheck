...
layui.config({
    ...
    function medicalCost(){
    	table.render({
            elem: '#medicalTable'
            , url: $WEB_ROOT_PATH + '/dhccApi/coststatistics/costStatistics/medicalCost'
            , cellMinWidth: 80
            , height: document.documentElement.clientHeight-65
            , cols: [[
                  {type: 'numbers', title: '序号'}
                , {field: 'itemCode', title: '编码', hide: true}
                , {field: 'medicalName', title: '药品名称',align: 'center'}
                , {field: 'totalNum', title: '药品数量',align: 'center'}
      ...
      require(
          ...
          function (ec) {
              ...
              var myChart = ec.init(document.getElementById('mainTwo')); 
           		...
           		// 为echarts对象加载数据
                $.ajax({
             	     url: $WEB_ROOT_PATH+"/dhccApi/coststatistics/costStatistics/getMedicalCostByItemCode?itemCode="+itemCode+"&diagType="+diagType+"&applyPayLevel="+applyPayLevel+"&handdingCode="+handdingCode+"&orgCode="+orgCode,
     		         type : "post",		
     ...
     require(
         ...
         function (ec) {
             ...
             var myChart = ec.init(document.getElementById('mainTwo')); 
          		...
          		// 为echarts对象加载数据
               $.ajax({
            	     url: $WEB_ROOT_PATH+"/dhccApi/coststatistics/costStatistics/getMedicalCostByItemCode?itemCode="+firstItemCode,
    		         type : "post",		
    		         async : true,		//异步请求（同步请求将会锁住浏览器，用户其他操作必须等待请求完成才可以执行）
    		         dataType : "json",	
    		         success : function(result) {
     ...
     require(
         ...
         function (ec) {
             ...
             var myChart = ec.init(document.getElementById('mainTwo')); 
          		...
          		// 为echarts对象加载数据
               $.ajax({
            	     url: $WEB_ROOT_PATH+"/dhccApi/coststatistics/costStatistics/getMedicalCostByItemCode?itemCode="+firstItemCode+"&diagType="+diagType+"&applyPayLevel="+applyPayLevel+"&handdingCode="+handdingCode+"&orgCode="+orgCode,
    		         type : "post",		
    		         async : true,		//异步请求（同步请求将会锁住浏览器，用户其他操作必须等待请求完成才可以执行）
    		         dataType : "json",	
    		         success : function(result) {
