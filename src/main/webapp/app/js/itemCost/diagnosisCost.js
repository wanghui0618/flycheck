...
layui.config({
    ...
    function diagnosisCost(){
    	table.render({
            elem: '#diagnosisTable'
            , url: $WEB_ROOT_PATH + '/dhccApi/coststatistics/costStatistics/diagnosisCost'
            , cellMinWidth: 80
            , height:document.documentElement.clientHeight-65
            , cols: [[
                  {type: 'numbers', title: '序号'}
                , {field: 'itemCode', title: '编码', hide: true}
                , {field: 'medicalName', title: '诊疗项目名称',align: 'center'}
                , {field: 'totalNum', title: '诊疗项目数量',align: 'center'}
    ...
    form.on('submit(LAY-user-front-search)', function (data) {
      ...
      //执行重载
        layui.table.reload('diagnosisTable', {
            where: field
            ,page: { curr: 1}
        	 ,done: function(res, curr, count){
            //如果是异步请求数据方式，res即为你接口返回的信息。
    ...
    //监听行单击事件
 	table.on('row(diagnosisTable)', function(obj){
 		//点击获取行的itemCode
     ...
     require(
         ...
         function (ec) {
             ...
             var myChart = ec.init(document.getElementById('mainTwo')); 
          		...
          		// 为echarts对象加载数据
               $.ajax({
            	     url: $WEB_ROOT_PATH+"/dhccApi/coststatistics/costStatistics/getDiagnosisCostByItemCode?itemCode="+itemCode+"&diagType="+diagType+"&applyPayLevel="+applyPayLevel+"&handdingCode="+handdingCode+"&orgCode="+orgCode,
    		         type : "post",		
     ...
     require(
         ...
         function (ec) {
             ...
             var myChart = ec.init(document.getElementById('mainTwo')); 
          		...
          		// 为echarts对象加载数据
               $.ajax({
            	     url: $WEB_ROOT_PATH+"/dhccApi/coststatistics/costStatistics/getDiagnosisCostByItemCode?itemCode="+firstItemCode,
    		         type : "post",		
    		         async : true,		//异步请求（同步请求将会锁住浏览器，用户其他操作必须等待请求完成才可以执行）
    		         dataType : "json",	
    		         success : function(result) {
     ...
     require(
         ...
         function (ec) {
             ...
             var myChart = ec.init(document.getElementById('mainTwo')); 
          		...
          		// 为echarts对象加载数据
               $.ajax({
            	     url: $WEB_ROOT_PATH+"/dhccApi/coststatistics/costStatistics/getDiagnosisCostByItemCode?itemCode="+firstItemCode+"&diagType="+diagType+"&applyPayLevel="+applyPayLevel+"&handdingCode="+handdingCode+"&orgCode="+orgCode,
    		         type : "post",		
    		         async : true,		//异步请求（同步请求将会锁住浏览器，用户其他操作必须等待请求完成才可以执行）
    		         dataType : "json",	
    		         success : function(result) {
