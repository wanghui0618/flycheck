...
function totle(formDate) {
	$
			.ajax({
				url : $WEB_ROOT_PATH
						+ '/dhccApi/sametimeHospitalrest/getTotalNumberOfCasesAndTotalAmount',
				type : "POST",
				data : formDate,
				dataType : "json",
				async : true,
...
function Detaildg(formDate) {
	...
	table
			.render({
				...
				loading : true,
				url : $WEB_ROOT_PATH
						+ '/dhccApi/sametimeHospitalrest/getSametimeHospital',
				cellMinWidth : 80,
	...
	table.on('tool(dg)',function(obj) { // 注：tool 是工具条事件名，test 是 table 原始容器的属性
						...
						if (layEvent === 'detail') { // 查看
							...
							layer.open({
										...
										area : [ '1100px', '550px' ],
										title : '病例详情',
										content : $WEB_ROOT_PATH + '/sametimeHospital/sametimeHospitalDetails',// 这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content:
																													// ['http://sentsin.com',
																													// 'no']
...
function downLoadXsl(fileName) {
	...
	form.attr('action', $WEB_ROOT_PATH
			+ '/dhccApi/sametimeHospitalrest/downLoadFile');
...
$("#export").click(
		function() {
			...
			$.ajax({
				url : $WEB_ROOT_PATH
						+ "/dhccApi/sametimeHospitalrest/expDate",
				type : "POST",
				data : formDate,
				dataType : "json",
				async : true,
