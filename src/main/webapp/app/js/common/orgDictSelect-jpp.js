$(function () {
    ...
    .keydown(function (e) {
        if (e.keyCode == 13) {
            ...
            //将查询条件存入隐藏域
            $('#orgName').val(keyValue)
        }
    }); 
     $('#getOrgName').combogrid('textbox').keyup(function(){
    	 var keyValue = $('#getOrgName').combogrid('textbox').val();
    	 $('#getOrgName').combogrid("setValue", keyValue);
     });
     $(function(){
    ...
    if (pager) {
        $(pager).pagination({
             ...
             onSelectPage: function (pageNumber, pageSize) { //按分页的设置取数据
            	var url=$WEB_ROOT_PATH +"/dhccApi/findDict/findDict/findDict?dictName=org";
                ...
                $('#getOrgName').combogrid("setValue", $('#orgName').val());
            },
            ...
            onRefresh: function (pageNumber, pageSize) { //点击刷新的处理
                $('#getOrgName').combogrid("setValue", $('#orgName').val());//将隐藏域中存放的查询条件显示在combogrid的文本框中
            }
        });
    }
