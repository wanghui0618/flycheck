...
function saveDb(){
	...
	console.log(medicalAduitId);
	console.log(medicalId);
	var url = $WEB_ROOT_PATH + "/dhccApi/medicalaudit/medicalAudit/save?medicalId1="+medicalId+"&medicalAduitId="+medicalAduitId;;
	dataAll.field["medicalAudit.fileurl"]=fileUrl;
	dataAll.field["medicalAudit.filenameurl"]=originalFilename;
	dataAll.field["medicalAudit.medicalId"]=medicalId;
	dataAll.field["medicalAudit.id"]=id;
	$("#filelist").val(originalFilename);

    $.post(url, dataAll.field, function (result) {
    	console.log(result);
    	console.log(dataAll.field);
    	 var inFlag= result.inFlag; 
    	 if(medicalAduitId!=null){
    		  if(inFlag==0){
    	   		  window.parent.closeLayer1('修改成功!');
    	   		  }else{
    	   	   	window.parent.closeLayer1('修改失败!');
    	   		  }
    	 }else{
    		  if(inFlag==0){
