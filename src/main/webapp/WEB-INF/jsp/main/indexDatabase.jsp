...
<body class="layui-layout-body">
			...
			<div class="layui-header"
				...
				<ul class="my-head-left">
					...
					<li style="margin-left: 5px; font-size: 15px; color: #fff"
						class="layui-nav-item">基础资料系统<span id="textHidden">&nbsp;>&nbsp;主页</span></li>
				</ul>
				<ul class="layui-nav layui-layout-right layui-nav-title"
	...
	<script>
		...
		function showClinicalPathWayView(data) {
			layer.open({
				offset : 'auto',
				area : [ "900px", "600px" ],
				type : 1,
				maxmin : true,
				title : data.name,
				content : '<div style="padding: 20px 30px;" >' + data.content+ '</div>',
				shade : 2
			});
		}
		...
		function showKnowledgeView(data) {
			layer.open({
				offset : 'auto',
				area : [ "900px", "600px" ],
				type : 1,
				maxmin : true,
				title : data.title + "     (关键字：" + data.keywords + ")",
				content : '<div style="padding: 20px 30px;" >' + data.content+ '</div>',
				shade : 2
			});
			var url = $WEB_ROOT_PATH + "/dhccApi/knowledge/knowledge/addViews";
			var id = data.id;
			$.post(url, {
				"knowledge.id" : id
			}, function(result) {
		...
		function drugInteraction(data) {
			layer.open({
						type : 2,
						title : data.drugName + '-相互作用',
						content : $WEB_ROOT_PATH
								+ '/druginteraction/druginfo3?drugInteraction.drugName='
								+ data.drugName,
						maxmin : true,
						area : [ '900px', '600px' ],
						success : function(layero, index) {
