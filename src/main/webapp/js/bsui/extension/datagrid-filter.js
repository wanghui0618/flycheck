(function($){
	...
	$.fn.datagrid.defaults.operators = {
		...
		contains: {
			text: '包含',
			isMatch: function(source, value){
				return source.toString().toLowerCase().indexOf(value.toLowerCase()) >= 0;
			}
		},
		equal: {
			text: '等于',
		...
		endwith: {
			text: '以..结尾',
			isMatch: function(source, value){
				return source.toString().toLowerCase().indexOf(value.toLowerCase(), source.length - value.length) !== -1;
			}
		},
		less: {
			text: '小于',
	...
	function init(target, filters){
		...
		function createFilterButton(container, operators){
			...
			btn.bind('click', {menu:menu}, function(e){
				...
				$(e.data.menu).menu('show', $(e.target).offset());
				return false;
			});
			return menu;
		}
